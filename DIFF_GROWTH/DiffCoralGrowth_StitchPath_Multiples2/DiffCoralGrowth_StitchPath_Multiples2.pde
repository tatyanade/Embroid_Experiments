...
float _maxSpeed = .7; // Maximum speed
...
void setup() {
  ...
  _diff_line = addDiffCircle(width/2, height/2, 90, 4);
  _diff_line2 = addDiffCircle(width/2, height/2, 100, 30);
  //_diff_line3 = addDiffCircle(width/2+random(-10,10), height/2+random(-10,10),120,8);
  //diff_line4 = new DifferentialLine();// addDiffCircle(width/2+random(-10,10), height/2+random(-10,10), 140,10);
}
...
DifferentialLine addDiffCircle(float cx, float cy, float rad, float desSep) {
  DifferentialLine _diff_line = new DifferentialLine(_maxForce, _maxSpeed, desSep, _separationCohesionRation, _maxEdgeLen);
  float nodesStart = 20;
  float angInc = TWO_PI/nodesStart;
  for (float a=0; a<TWO_PI; a+=angInc) {

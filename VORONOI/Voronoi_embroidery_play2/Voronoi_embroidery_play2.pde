...
void draw() {
  if (!runningEmbroidery) {
    ...
    if (!ranEmbroidery) {
      ...
      saveFrame(str(frame)+"voronoi.png");
      frame++;
      writePembroider(E2,"Voronoi_Softened");
    }
    ranEmbroidery = true;
  }
}
...
String timeStamp() {
  return "D" + str(day())+"_"+str(hour())+"-"+str(minute())+"-"+str(second())+"-FRAME"+str(frameCount);
}

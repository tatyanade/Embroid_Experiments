...
void setup() {
  ...
  image_EXT = loadImage("TEST14.png"); 
...
void draw() {
  if (edditting) {
    ...
    background(180);
    E_EXT.visualize(true, true, true, frame*5);
...
void renderZigPolyFUN_OUTER(PEmbroiderGraphics E, PEmbroiderGraphics E_ref, int i, float stWidth, int dir) {
  ...
  for (int k=0; k<E_ref.polylines.get(i).size()-1; k++) {
    ...
    for (int j = 0; j <increments; j++) {
      t = (float(k)+kSteps*float(j))/float(E_ref.polylines.get(i).size()-1);
      if (f1(t)>sizeFilter) {
        E.line(p1.x+step.x*j+(tan.x*dir*f1(t)), p1.y+step.y*j+(tan.y*dir*f1(t)), p1.x+step.x*j+tan.x*dir*f1(t)*2, p1.y+step.y*j+tan.y*dir*f1(t)*2);
        E.line(p1.x+step.x*j+tan.x*dir*f1(t)*2, p1.y+step.y*j+tan.y*dir*f1(t)*2, p1.x+step.x*j+(tan.x*dir*f1(t)), p1.y+step.y*j+(tan.y*dir*f1(t)));
      } 
      dir*= -1;
    }
  }
...
void renderZigFUNOnLines(PEmbroiderGraphics E, PEmbroiderGraphics E_ref) {
  for (int i=0; i<E_ref.polylines.size(); i++) {
    if (E_ref.polylines.get(i).size() >= 8) {
      renderZigPolyFUNProf(E, E_ref, i, hatchSpacing/8, 1, true);
      renderZigPolyFUNProf_REV(E, E_ref, i, hatchSpacing*3/8, 1, false);
      E.beginOptimize();
      renderZigPolyFUN_OUTER(E, E_ref, i, hatchSpacing*4/8, 1);
      E.endOptimize();
      polyTraced++;
    }
  }

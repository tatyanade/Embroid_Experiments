  ...
  for (int i = -cols; i<cols; i++) {
    if (i%2 == 0) {
      for (int j = -rows; j<rows; j++) {
        int x = i*offset;
        int y = j*offset;
        PVector vect = E.HATCH_VECFIELD.get(x, y).div(2);
        doubleStitch(E, x, y, x+vect.x, y+vect.y,1);
      }
    } else {
...
void doubleStitch(PEmbroiderGraphics E, float x, float y, float x2, float y2, int n) {
  if(n>5){
    return;
  }
  E.line(x, y, x2, y2);
  PVector vect = E.HATCH_VECFIELD.get(x2, y2).div(2);
  doubleStitch(E, x2, y2, x2+vect.x, y2+vect.y,n+1);
  E.line(x2, y2, x, y);
}
  ...
  public PVector get(float x, float y) {
    ...
    case 1:
     // println(gradient(x,y));
      return gradient(x,y).normalize().mult(10);
    case 2: 

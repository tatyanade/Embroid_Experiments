...
void renderZigPoly(PEmbroiderGraphics E, PEmbroiderGraphics E_ref, int i, float stWidth, int increments, int dir) {
  E.beginShape();
  println(E_ref.polylines.size());
  for (int k=0; k<E_ref.polylines.get(i).size()-1; k++) {
    PVector p1 = E_ref.polylines.get(i).get(k);
...
void renderZigPolyCircProf(PEmbroiderGraphics E, PEmbroiderGraphics E_ref, int i, float stWidth, int increments, int dir) {
  E.beginShape();
  println(E_ref.polylines.size());
  for (int k=0; k<E_ref.polylines.get(i).size()-1; k++) {
    PVector p1 = E_ref.polylines.get(i).get(k);
...
void renderZigOnLines(PEmbroiderGraphics E, PEmbroiderGraphics E_ref) {
  for (int i=0; i<E_ref.polylines.size(); i++) {
    renderZigPoly(E, E_ref, i, 10,3,1);
  }
}

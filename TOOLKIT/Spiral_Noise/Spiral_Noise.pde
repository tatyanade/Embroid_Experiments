...
float offset(float r, float theta, int i) {
 ...
 // float bright = getPixelBrightness(img,int(coord.x),int(coord.y));
  float off = logiMap(noise(coord.x*.01,coord.y*.01, z/20));////(255-bright)/255*offset;// offset amount goes down as the brightness goes up (neg, lin proportionality)
  println(off);
  if(off < .1){
    off = 0;
  }
...
float offset_Inv(float r, float theta, int i) {
  PVector coord = radi2card(r,theta).add(new PVector(width/2,height/2));
  float bright = getPixelBrightness(img,int(coord.x),int(coord.y));

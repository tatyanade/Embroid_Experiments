...
int firstDiam = 150;//80int(random(50,70));
color mossyColor = color(255);//color(int(random(255)),int(random(255)),int(random(255)));
...
void setup() {
  ...
  E.stroke(245, 233, 66);
  drawKnot1(E, firstDiam - 20, width/2, height/2);
  E.beginOptimize();
  E.noStroke();
  E.hatchMode(E.CROSS);
  E.hatchSpacing(10);
  E.fill(245, 233, 66);
  E.circle(width/2, height/2,firstDiam*2-20);
  E.endOptimize();
  E.beginOptimize();
  E.hatchSpacing(3);
  E.circle(width/2, height/2,firstDiam*2);
  E.endOptimize();
...
void drawDoublMossyCirlce(PEmbroiderGraphics E, int x, int y, float z, int diamOffset) {
  ...
  drawMossyCircle(E, x, y, firstDiam, z, 0, 2, centOff, diamOffset);
//  drawMossyCircle(E, x, y, firstDiam, z, 20, 1.5, centOff, diamOffset);
  drawMossyCircle(E, x, y, firstDiam, z, 25, 1, centOff, diamOffset);
}
...
void drawKnot1(PEmbroiderGraphics E, float rad, float x, float y) {
  ...
  drawStarKnot(E, int(rad-7*2), 20);

  drawStarKnot(E, int(rad-7), 10);

  E.rotate(PI/11);
  drawStarKnot(E, int(rad), 7); // .85
  E.popMatrix();
}

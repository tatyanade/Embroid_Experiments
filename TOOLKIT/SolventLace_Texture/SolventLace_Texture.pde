...
PImage img;  // INTERIOR CIRCLE
PImage img2; // CUT LINE
PImage img3; // EXTERIOR LACY BITS
...
void setup() {
  ...
  E.noFill();
  E.stroke(0);
  renderZigPolyFUNProf(E,img,200);
...
void renderZigPolyFUNProf(PEmbroiderGraphics E, PImage img, float stWidth) {
  ...
  for (int k=0; k<E_ref.polylines.get(i).size()-1; k++) {
    ...
    PVector perp = tan.copy().rotate(PI/2).mult(-20);;
    tan.rotate(PI/30);
    tan.mult(stWidth/2);
    p1.add(perp);

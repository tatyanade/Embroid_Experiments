...
void draw() {
  ...
  stroke(255, 255, 255);
  for (int i = 0; i < nGestures; i++) {
    renderGesture(gestureArray[i], width, height); // render the nth gesture
  }
}
...
void renderGesture(Gesture gesture, int w, int h) {
  if (gesture.exists) {
    if (gesture.nPolys > 0) {
      ...
      //beginShape(QUADS);
      Vec3f P_0 = gesture.path[0];
    //  println(P_0.x,P_0.y);
      pushStyle();
      stroke(255);
      point(P_0.x,P_0.y);
      popStyle();
      int gnp = gesture.path.length;
      println(gnp);
      for (int i=0; i<gnp-1; i++) {
        Vec3f P0 = gesture.path[i];
        Vec3f P1 = gesture.path[i+1];
        //p = polygons[i];
       // xpts = p.xpoints;
       // ypts = p.ypoints;
       ...
       // endShape();
       PVector TEST = new PVector(P0.x-P1.x,P0.y-P1.y);
       if(!(P1.x == 0 && P1.y==0)){
